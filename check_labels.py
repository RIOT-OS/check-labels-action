...
VALID_CONDITIONS = {
    'review.approvals': check_review_approvals
}
    ...
    for pull_label in pull_labels:
        ...
        for unset_label in unset_labels:
            if fnmatch.fnmatch(pull_label, unset_label):
                print("{} are expected not to be set"
                      .format(", ".join(unset_labels)))
                return 1
    if not all(set_labels_check):
        print("{} are expected to be set"
              .format(", ".join(set_labels)))
        return 1
    res = 0
    for cond_label, condition in cond_labels:
        for pull_label in pull_labels:
            if fnmatch.fnmatch(pull_label, cond_label) and \
               not check_condition(pull, condition):
                print("Condition {} for label {} not fulfilled"
                      .format(condition, pull_label))
...
def main():
    ...
    parser.add_argument(
        'set_labels', default='', type=cs_string_list,
        help="Comma-seperated list of labels required to be set. "
             "default: ''"
    )
    parser.add_argument(
        'unset_labels', default='', type=cs_string_list,
        help="Comma-seperated list of labels required not to be set. "
             "default: ''"
    )
    parser.add_argument(
        'cond_labels', default='', type=cs_2tuple_list,
    ...
    repo = github.Github(
        os.environ.get("INPUT_ACCESS_TOKEN", None)
    ).get_repo(repo_name)

    return check_labels(args.set_labels, args.unset_labels, args.cond_labels,
...
if pytest:
    ...
    def test_check_condition(monkeypatch):
        pull = MockPull()
        monkeypatch.setattr(sys.modules[__name__], "parse_condition",
                            lambda x: ["test", 1])
    ...
    @pytest.mark.parametrize(
        ...
        [([], [], [], False, [], 0),
         ...
         ([], ["don't merge"], [("foobar", "test>1")],
          False, [], 0),
         ([], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala"], 0),
         ([], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar"], 1),
         ([], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar", "don't merge"], 1),
         ([], ["don't merge"], [("foobar", "test>1")],
          True, [], 0),
         ([], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala"], 0),
         ([], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar"], 0),
         ([], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar", "don't merge"], 1),
         (["yes"], [], [], False, [], 1),
         (["yes"], [], [], False, ["lalala"], 1),
         ...
         (["yes"], ["don't merge"], [], True,
          ["lalala", "don't merge", "yes"], 1),
         (["ye*"], ["don't merge"], [("foobar", "test>1")],
          False, [], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar", "don't merge"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar", "don't merge", "yes"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          False, ["lalala", "foobar", "yes"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, [], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar", "don't merge"], 1),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar", "yes"], 0),
         (["yes"], ["don't merge"], [("foobar", "test>1")],
          True, ["lalala", "foobar", "don't merge", "yes"], 1)]
    )
    # pylint: disable=too-many-arguments
    def test_check_labels(monkeypatch, set_labels, unset_labels, cond_labels,
                          cond_res, pull_labels, exp):
        pull = MockPull(labels=[MockLabel(name) for name in pull_labels])
        monkeypatch.setattr(sys.modules[__name__], "check_condition",
                            lambda x, y: cond_res)
    ...
    def test_main(monkeypatch):
        ...
        monkeypatch.setattr(argparse.ArgumentParser, "parse_args",
                            lambda *args, **kwargs: MockArgs())
        monkeypatch.setattr(github, "Github",
                            lambda *args, **kwargs: MockGithub())
        monkeypatch.setattr(sys.modules[__name__], "check_labels",
                            lambda *args, **kwargs: 0)
        monkeypatch.setattr(sys.modules[__name__], "get_pull_no",
